...
extern IDirect3DTexture9 * tRA3ICONS;
extern ID3DXSprite * sRA3ICONS;
...
{
	UninitializeDX( gl_pmyIDirect3DDevice9 );
	render->Invalidate();
	texturesInitResources( gl_pmyIDirect3DDevice9, false );
	bD3DRenderInit = false;
}

void proxyID3DDevice9_InitOurShit ( HWND hDestWindowOverride )
{	
	render->Initialize( gl_pmyIDirect3DDevice9 );
	InitializeDX( gl_pmyIDirect3DDevice9 );
	OnGameLaunch();
	texturesInitResources( gl_pmyIDirect3DDevice9, true );
	g_WindowHandle = hDestWindowOverride;
	bD3DRenderInit = true;
}
...
HRESULT myIDirect3DDevice9::Present(CONST RECT* pSourceRect,CONST RECT* pDestRect,HWND hDestWindowOverride,CONST RGNDATA* pDirtyRegion)
{
	...
	//this->ShowWeAreHere();
	if ( !bD3DRenderInit )
		proxyID3DDevice9_InitOurShit( hDestWindowOverride );
	//bool BeganScene = gl_pmyIDirect3DDevice9->BeginScene();
	bool isBeginRenderWIN = SUCCEEDED( render->BeginRender() );
	if ( isBeginRenderWIN )
	{
		FrameTick(gl_pmyIDirect3DDevice9, hDestWindowOverride);
		render->EndRender();
	}
	//if(BeganScene)
	//	gl_pmyIDirect3DDevice9->EndScene();
...
void myIDirect3DDevice9::ShowWeAreHere(void)
{
	...
	m_pIDirect3DDevice9->Clear(1, &rec, D3DCLEAR_TARGET, D3DCOLOR_ARGB(255,255,255,0),-9999 ,0);
